...
import { Download, Plus, Trash2, FileText, Upload, X, DollarSign, CheckCircle, Send, Edit, MoreVertical, Palette, Save } from 'lucide-react';
...
export default function InvoiceApp() {
  ...
  const generateInvoicePDF = (invoice: Invoice) => {
    try {
      ...
      const yellowBg: [number, number, number] = [254, 243, 199]; // Yellow background for PARTIAL
      ...
      if (logo) {
        try {
          doc.addImage(logo, 'PNG', margin, yPos, 35, 35);
        } catch (e) {
          console.error('Logo error:', e);
        }
      }
  
      // Company Name (large, below or next to logo)
      yPos += 45;
      doc.setFontSize(18);
      ...
      if (companyWebsite) {
        yPos += 4;
        doc.text(companyWebsite, margin, yPos);
      }
      ...
      if (invoice.status !== 'draft') {
        ...
        switch (invoice.status) {
          case 'paid':
            statusBgColor = [220, 252, 231]; // Green
            statusTextColor = [21, 128, 61];
            break;
          case 'partial':
            statusBgColor = [254, 243, 199]; // Yellow
            statusTextColor = [161, 98, 7];
            break;
          case 'overdue':
            statusBgColor = [254, 226, 226]; // Red
            statusTextColor = [185, 28, 28];
            break;
          default:
            statusBgColor = [219, 234, 254]; // Blue
            statusTextColor = [30, 64, 175];
        }
        
        // Draw status badge
        doc.setFillColor(...statusBgColor);
        doc.roundedRect(pageWidth - 70, statusY, 50, 10, 3, 3, 'F');
        
        doc.setFontSize(10);
        doc.setTextColor(...statusTextColor);
        doc.setFont('helvetica', 'bold');
        doc.text(statusText, pageWidth - 45, statusY + 7, { align: 'center' });
      }
  
      // ========== INVOICE DETAILS BOX (RIGHT SIDE) ==========
      
      const detailsY = margin + 40;
      ...
      // Invoice Number
      doc.text('Invoice #:', labelX, detailsY);
      doc.setFont('helvetica', 'normal');
      doc.text(invoice.invoiceNumber, pageWidth - margin, detailsY, { align: 'right' });
      
      // Date
      doc.setFont('helvetica', 'bold');
      doc.text('Date:', labelX, detailsY + 6);
      doc.setFont('helvetica', 'normal');
      doc.text(new Date(invoice.invoiceDate).toLocaleDateString('en-GB'), pageWidth - margin, detailsY + 6, { align: 'right' });
      
      // Due Date
      doc.setFont('helvetica', 'bold');
      doc.text('Due Date:', labelX, detailsY + 12);
      doc.setFont('helvetica', 'normal');
      doc.text(new Date(invoice.dueDate).toLocaleDateString('en-GB'), pageWidth - margin, detailsY + 12, { align: 'right' });
  
      // ========== BILL TO SECTION ==========
      
      yPos = Math.max(yPos + 15, detailsY + 25);
      checkNewPage();
      ...
      // Table Header (Blue background like web)
      ...
      doc.text('Description', margin + 5, yPos + 6.5);
      doc.text('Qty', pageWidth - 100, yPos + 6.5, { align: 'center' });
      doc.text('Unit Price', pageWidth - 65, yPos + 6.5, { align: 'right' });
      doc.text('Amount', pageWidth - margin - 5, yPos + 6.5, { align: 'right' });
      ...
      invoice.items.forEach((item, index) => {
        ...
        doc.text(desc, margin + 5, yPos + 5);
        doc.text(item.quantity.toString(), pageWidth - 100, yPos + 5, { align: 'center' });
        doc.text(`GHS ${item.unitPrice.toFixed(2)}`, pageWidth - 65, yPos + 5, { align: 'right' });
        doc.text(`GHS ${item.amount.toFixed(2)}`, pageWidth - margin - 5, yPos + 5, { align: 'right' });
      ...
      doc.text(`GHS ${invoice.subtotal.toFixed(2)}`, pageWidth - margin - 5, totalsY, { align: 'right' });
      totalsY += 8;
  
      // Tax (if applicable)
      if (invoice.taxRate > 0) {
        doc.text(`Tax (${invoice.taxRate}%)`, totalsX, totalsY);
        doc.text(`GHS ${invoice.taxAmount.toFixed(2)}`, pageWidth - margin - 5, totalsY, { align: 'right' });
        totalsY += 8;
      }
      ...
      doc.text(`GHS ${invoice.total.toFixed(2)}`, pageWidth - margin - 5, totalsY, { align: 'right' });
      totalsY += 12;
      ...
      if (totalPaid > 0) {
        ...
        doc.text(`GHS ${totalPaid.toFixed(2)}`, pageWidth - margin - 5, totalsY, { align: 'right' });
        totalsY += 10;
      }
  
      // Balance Due (highlighted)
      const balance = invoice.total - totalPaid;
      if (balance !== invoice.total) { // Only show if there are payments
        doc.setFontSize(12);
        doc.setFont('helvetica', 'bold');
        doc.setTextColor(balance > 0 ? 220 : 34, balance > 0 ? 38 : 197, balance > 0 ? 38 : 94);
        
        doc.text('Balance Due:', totalsX, totalsY);
        doc.text(`GHS ${balance.toFixed(2)}`, pageWidth - margin - 5, totalsY, { align: 'right' });
      }
  
      // ========== PAYMENT TERMS (BOTTOM LEFT) ==========
      
      if (invoice.terms || invoice.notes) {
        yPos = totalsY - 40; // Position parallel to totals
        checkNewPage();
        ...
        if (invoice.terms) {
          const terms = doc.splitTextToSize(invoice.terms, 90);
          doc.text(terms, margin, yPos);
          yPos += terms.length * 4.5;
        }
        
        if (invoice.notes) {
          yPos += 3;
          const notes = doc.splitTextToSize(invoice.notes, 90);
      ...
      doc.text(`Generated on ${new Date().toLocaleDateString('en-GB')}`, pageWidth / 2, pageHeight - 15, { align: 'center' });
